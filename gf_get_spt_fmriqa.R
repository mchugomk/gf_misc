...
setwd("/data/gf")
gf_scan_info <- read.csv("/data/gf/gf_xnatreport.csv", skip = 2) %>%
  filter(as_type=="spt1APA_FMRI_MB3_2_5mm_1300")
gf_scan_info$as_label <- as.character(gf_scan_info$as_label)
gf_fmriqa_info <- read.csv("/data/gf/Xnatreport_fmriqa_v4_info.csv")
...
gf_spt_fmriqa <- left_join(gf_scan_info, gf_fmriqa_info, by=c("project_id"="project_label","subject_label","session_label",
                                                              "as_label"="scan_fmri"))
write.csv(gf_spt_fmriqa, "/data/gf/gf_spt_fmriqa_v4.csv",row.names = F)

# merge fmriqa with gf fmri processed data
gf_spt_proc_info <- read.csv("/data/gf/gf_spt_processed.csv")
...
write.csv(gf_spt_proc_info_fmriqa, "/data/gf/gf_spt_processed_fmriqa.csv",row.names = F)
